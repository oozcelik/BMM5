...
/*    for (int iC=0; iC< fpEvt->nGenCands(); iC++){
	pC = fpEvt->getGenCand(iC);
        double dR(-99.);
        for (int i = 0; i < fpEvt->nSimpleTracks(); ++i) {
             double pT(0), deltaR(0), recoPt(0), GenPt(0);
             int idx = fpEvt->getSimpleTrack(i)->getGenIndex();
              sTrk = 0;
              sTrk = fpEvt->getSimpleTrack(i);	
              GenPt = pC->fP.Perp();
              ...
              deltaR = (sTrk->getP()).DeltaR(pC->fP.Vect());
              if (dR<deltaR) dR = deltaR;
              else continue;
              cout << " gen cand " << iC << " cand reco " << i << " idx " << idx << " " << pC->fID << endl;
       } 
    }*/
...
//	cout << "no TAnaMuon found despite fMu1TmId != 0!!" << endl;
	continue;
        ...
        GenPt = pC->fP.Perp();
        recoPt = pt->fPlab.Perp();
        pT = abs(GenPt-recoPt)/recoPt;
        deltaR = (pt->fPlab).DeltaR(pC->fP.Vect());
       // cout << " PT " << pT << " deltaR " << deltaR << endl;
     
//       cout << " reco muon?? " << pt->fIndex << endl;  
        
        /*       if (pt > 0){
         cout << " pt " << pt->fPlab.Perp() << " eta " << pt->fPlab.Eta() << " " << pt->fMuID << " " << pC->fID << endl;
       }*/

     cout << " p1->fMuIndex " << pt <<  " pID " << pC->fID << " mother id " << TMath::Abs(pM->fID) <<  endl;

     int momID = TMath::Abs(pM->fID);
     int muflag;

     if ((pt->fMuID & 2) == 2)  muflag = 1;
     else muflag = -99;
     //cout << i << " " << muflag << " " << fMu1GmId << " " << fMu2GmId << endl;
...
//  t->Branch("segComp",              &fMuonData.segComp,  "segComp/F");
  ...
  t->Branch("pID",                   &fMuonData.pID,  "pt/F");
  t->Branch("MomID",                 &fMuonData.MomID, "eta/F");
