...
candAna::candAna(bmmReader *pReader, string name, string cutsFile) {
  ...
  cout << "======================================================================" << endl;
  cout << "==> candAna: name = " << name << ", " << cutsFile << ", year " << fYear << ", era ->" << fEra << "<-" << fpReader << " fpReader " << endl;
...
void candAna::evtAnalysis(TAna01Event *evt) {
  ...
  fTIS = fTOS = fGoodHLT1 = fGoodRareHLT1 = fRefTrigger = false;
...
//      cout << " filling muon ID tree " << endl; 
      fMuonIdTree->Fill();
      fTree->Fill();
...
void candAna::candAnalysis() {
  ...
  if (0 && fCandDcand && fDcandIdx > -1) {
    ...
    for (int i = fpCand->fSig1; i <= fpCand->fSig2; ++i) {
      ...
      for (int j = dCand->fSig1; j <= dCand->fSig2; ++j) {
...
//  cout << " checking candidate type!!! " << fCandType << endl;
  ...
  fMu1GmId      = ((p1->fMuID & 2) == 2); //global muon
  ...
  if (1) {
   ...
   /// not necessary, just to check ///
    ...
    TAnaMuon *pt(0), *pp(0), *pp2(0);
    TGenCand *pC(0);
    TGenCand *pM(0), *pMom(0), *ptest(0);
    TSimpleTrack *sTrk(0);


    TAnaTrack *q1(0), *q2(0), *t1(0), *t2(0);
    TAnaCand *pCand(0);
...
//    cout << fRecM1Tmi << " " << fRecM2Tmi << endl;
	...
	if (idx > -1 && idx < fpEvt->nMuons()) {
	  pt = fpEvt->getMuon(idx);
          pC = fpEvt->getGenTWithIndex(fGenM1Tmi); // Gen matched candidate!
          pM = fpEvt->getGenTWithIndex(pC->fMom1); 
	}
       else continue;
      }
      if (1 == i) {
	...
	if (idx > -1 && idx < fpEvt->nMuons()) {
	  pt = fpEvt->getMuon(idx);
          pC = fpEvt->getGenTWithIndex(fGenM2Tmi); // Gen matched candidate!
          pM = fpEvt->getGenTWithIndex(pC->fMom1);
	}
       else continue;
      }
...
//         cout <<" dau2 " <<  it << " " << sTrk->getGenIndex() << " " << pC->fDau2 << " " << ig << endl;
  ...
  if((p1->fMuIndex >0 || p2->fMuIndex >0) && pt>0){
      cout << " THERE FAKE in the event " << fEvt << endl;
////////////////// set up variables for Muon MVA ///////////////////////////
//////  but remember here, muons can be fake, there is no selection for the real muons /////
...
/*   else if (p2->fMuIndex >0 && pt>0){
    cout << " there is fake " << endl;
      ...
      fMuonData.pID           = -999.;
      fMuonData.gmuid         = -999.;
      fMuonData.segComp       = -999.;
      fMuonData.chi2LocMom    = -999.;
      fMuonData.chi2LocPos    = -999.;
      fMuonData.NTrkVHits     = -999.;
      fMuonData.glbTrackTailProb  = -999.;
      fMuonData.glbDeltaEtaPhi    = -999.;
      fMuonData.trkValidFract     = -999.;
      fMuonData.LWH           = -999.;
      fMuonData.dxyRef        = -999.;
      fMuonData.dzRef         = -999.;
      fMuonData.kinkFinder    = -999.;
      fMuonData.glbKinkFinder = -999.;
      fMuonData.glbKinkFinderLOG = -999.;
      fMuonData.timeAtIpInOutErr  = -999.;
      fMuonData.outerChi2     = -999.;
      fMuonData.valPixHits    = -99.;
      fMuonData.TMTrkMult100     = -999.;
      fMuonData.innerChi2        = -999.;
      fMuonData.trkRelChi2       = -999.;
      fMuonData.vMuonHitComb     = -999.;
      fMuonData.Qprod            = -999.;
   }
}
...
//   fPreselection = fGoodMuonsPt && fGoodMuonsEta; 

  fPreselection = fPreselection && fGoodMuonsPt && fGoodJpsiCuts;
...
void candAna::triggerHLT() {
  ...
  if (HLTRANGE.begin()->first == "NOTRIGGER") {
    ...
    int rmin, rmax;
    bool good(false);
    for (map<string, pair<int, int> >::iterator imap = RARETRIGGERRANGE.begin(); imap != RARETRIGGERRANGE.end(); ++imap) {
      spath = imap->first;
...
void candAna::triggerL1T() {
  ...
  for (int i = 0; i < NL1T; ++i) {
    ...
    if (2017 == fYear) {
      if ("L1_DoubleMu0er1p5_SQ_OS_dR_Max1p4" == fpEvt->fL1TNames[i]) {
	fL1Seeds |= 0x1; //1
...
// ---------------------------------------------------------------------
// Just to fake the triggerSelection method for the moment. Used in candAnaRecoil()
...
void candAna::bookHist() {
  ...
  fEffTree->Branch("rhlt1",  &fGoodRareHLT1,      "rhlt1/O");
...
void candAna::setupMuonIdTree(TTree *t) {
  ...
  t->Branch("iValFrac",        &fMuonData.trkValidFract, "iValFrac/f");
  t->Branch("segComp",              &fMuonData.segComp,  "segComp/F");
  t->Branch("chi2LocMom",           &fMuonData.chi2LocMom, "chi2LocMom/F");
  t->Branch("chi2LocPos",           &fMuonData.chi2LocPos,  "chi2LocPos/F");
  t->Branch("NTrkVHits",            &fMuonData.NTrkVHits,  "NTrkVHits/F");
  t->Branch("glbTrackTailProb",     &fMuonData.glbTrackTailProb,  "glbTrackTailProb/F");
  t->Branch("glbDeltaEtaPhi",       &fMuonData.glbDeltaEtaPhi,  "glbDeltaEtaPhi/F");
  t->Branch("LWH",                  &fMuonData.LWH,  "LWH/I");
  t->Branch("dxyRef",               &fMuonData.dxyRef,  "dxyRef/F");
  t->Branch("dzRef",                &fMuonData.dzRef,  "dzRef/F");
  t->Branch("kinkFinder",           &fMuonData.kinkFinder,  "kinkFinder/F");
  t->Branch("glbKinkFinder",        &fMuonData.glbKinkFinder,  "glbKinkFinder/F");
  t->Branch("TMath::Log(2+glbKinkFinder)", &fMuonData.glbKinkFinderLOG,  "glbKinkFinderLOG/F");
  t->Branch("timeAtIpInOutErr",     &fMuonData.timeAtIpInOutErr,  "timeAtIpInOutErr/F");
  t->Branch("outerChi2",            &fMuonData.outerChi2,  "outerChi2/F");
  t->Branch("innerChi2",            &fMuonData.innerChi2,  "innerChi2/F");
  t->Branch("valPixHits",           &fMuonData.valPixHits, "valPixHits/I");
  t->Branch("TMTrkMult100",         &fMuonData.TMTrkMult100, "TMTrkMult100/F");
  t->Branch("trkRelChi2",           &fMuonData.trkRelChi2,  "trkRelChi2/F");
  t->Branch("vMuonHitComb",         &fMuonData.vMuonHitComb,  "vMuonHitComb/F");
  t->Branch("Qprod",                &fMuonData.Qprod, "Qprod/I");
  t->Branch("pt",                   &fMuonData.pt,  "pt/F");
  t->Branch("p",                   &fMuonData.p,  "p/F");
  t->Branch("eta",                  &fMuonData.eta, "eta/F");
  t->Branch("dr",                   &fMuonData.dr,  "dr/F");
  t->Branch("dpt",                   &fMuonData.dpt,  "dpt/F");
  t->Branch("pID",                   &fMuonData.pID,  "pID/F");
  t->Branch("MomID",                 &fMuonData.MomID, "momID/F");
  t->Branch("gmugmid",              &fMuonData.gmuid, "gmugmid/F");
  // removed rest
}
...
void candAna::setupReducedTree(TTree *t) {
  ...
  t->Branch("rhlt1",   &fGoodRareHLT1,      "rhlt1/O");
...
void candAna::readCuts(string fileName, int dump) {
  ...
  for (unsigned int i = 0; i < cutLines.size(); ++i) {
    ...
    if (!strcmp(CutName, "RARETRIGGERRANGE")) {
      char triggerlist[1000];
      sscanf(buffer, "%s %s", CutName, triggerlist);
      string tl(triggerlist);
      int r1(0), r2(0);
      string hlt = splitTrigRange(tl, r1, r2);
      RARETRIGGERRANGE.insert(make_pair(hlt, make_pair(r1, r2)));
      if (dump) {
	cout << "RARETRIGGERRANGE:       " << hlt << " from " << r1 << " to " << r2 << endl;
      }
    }
...
bool candAna::mvaMuon(TAnaMuon *pt, double &result, bool hadronsPass) {
  ...
  if (doNow) {
    ...
    if (TMath::Abs(pt->fPlab.Eta()) < 0.9) {
      ...
      //cout << "mrd.chi2LocMom = " << mrd.chi2LocMom << " mrd.chi2LocPos = " << mrd.chi2LocPos << endl;
      return (result > MUBDT);
    } else {
...
bool candAna::refTrigger(TAnaCand *pC, string refTriggerPath) {
  ...
  for (int i = 0; i < fpEvt->nTrgObjv2(); ++i) {  // loop over all saved hlt objects
    ...
    if (pTO->fHltPath.Contains(refTriggerPath)) {
      ...
      for (unsigned int j = 0; j < nTrgIdx; ++j) {
	double dr(0.);
...
// ----------------------------------------------------------------------
// -- calculate closest distance (deltaR) for a simpleTrack to a PD trigger object at L3 Level
// -- This is to check the distance of simpleTrack to any other L3 muon for Muon MVA validation
...
float candAna::gentorecomatch(TAnaMuon *muon, TGenCand *pC) {

    float GenPt (0), recoPt(0), deltaR(0), pT(0);
    for (int iC = 0; iC < fpEvt->nMuons(); ++iC) {
        pC = fpEvt->getGenTWithIndex(fGenM1Tmi);
        GenPt  = pC->fP.Perp();
        recoPt = muon->fPlab.Perp(); 
        pT = abs(GenPt-recoPt)/recoPt;
        deltaR = (muon->fPlab).DeltaR(pC->fP.Vect());
  }
//  cout << " PT " << pT << " deltaR " << deltaR << endl;
  return deltaR; 
}
