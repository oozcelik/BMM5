...
void candAna::evtAnalysis(TAna01Event *evt) {
  ...
  if (1233 == fVerbose) {
    ...
    for (int it = 0; it < fpEvt->nSimpleTracks(); ++it) {
     ...
     cout << " number of track "<< fpEvt->nSimpleTracks() << " fpEvt->getSimpleTrack(it) " << fpEvt->getSimpleTrack(it) << endl;
...
/*  bool match = false;
// check for the B+ truth-match 
  if (3000068 == fpCand->fType) {
//       cout << " FUCK " << fpCand->fType << endl;
       sv = fpCand->fVtx;
       cout << "Xaxis " << sv.fPoint.X() << " " << genSV.X() << endl;
       match = true;
  }
  if (!match) return;*/
  ...
  fGoodCmssw   =  (fMu1Pt > 4.) && (fMu2Pt > 4.) // 4GeV
    && (TMath::Abs(fCandPvIpS) < 5.)
    && (fCandFLSxy > 4.) // 4 
    && (TMath::Abs(fCandDoca) < 0.08) // 0.08
...
double candAna::osIsolation(TAnaCand *pC, double r, double ptmin) {
  ...
  for (int i = 0; i < fpEvt->nSimpleTracks(); ++i) {
    pT = fpEvt->getSimpleTrack(i);
    if (verbose) {
      cout << "   track " << i << " total number of tracks in the event " << fpEvt->nSimpleTracks() 
