...
void plotReducedOverlays::makeAll(string what) {
  ...
  if (what == "dbx") {
    ...
    system(Form("/bin/rm -f %s/plotSbsHistograms-%s.root", fDirectory.c_str(), fSuffix.c_str()));
    makeSampleOverlay("bupsikData", "bupsikMcComb", 1000000);
    return;
  }
...
//  bool cmsswPresel =  (fb.m1pt>4.) && (fb.m2pt>4.)  && (fb.pvips < 10.) && (fb.flsxy > 5) && (fb.maxdoca < 0.1);

  // Let's put it by default !!!
//  bool cmsswPresel =  (fb.m1pt>4.) && (fb.m2pt>4.) && (fb.flsxy > 4) && (fb.alpha < 0.1) && (fb.fls3d > 5) && (fb.pvips < 4); 

 //bool cmsswPresel =  (fb.m1pt>4.) && (fb.m2pt>4.) && (fb.flsxy > 8) && (fb.alpha < 0.08) && (fb.fls3d > 10) && (fb.pvips < 4) && (fb.chi2 < 5.) ;

  bool cmsswPresel =  (fb.m1pt>4.) && (fb.m2pt>4.) ;  
//&& (fb.pvips < 99.) && (fb.flsxy > 0) && (fb.maxdoca < 0.) && (fb.chi2 < 99.);
...
//  cout << " number of tracks------------> " << fb.pvntrk << " type of " << fb.type << " in the event--------> " << fb.evt <<endl;
...
//   cout << "fGoodHLT " << fGoodHLT << " fb.hlt1 " << fb.hlt1 << " fb.tos " << fb.tos << " fb.l1t " << fb.l1t << endl;

  double bdtCut(0.1);  /// 0.1 by default
  if (-1 < fChan && fChan < fNchan) {
    bdtCut = fCuts[fChan]->bdtCut;
  }
  // fPreselection    = (fGoodHLT && (fb.alpha < 0.1) && (fb.fls3d > 6) && (fb.pvips < 4) && (fb.docatrk < 0.2));
//   fPreselectionBDT = (fGoodHLT && (fBDT > 0.3));
  fPreselection    = fGoodHLT ;
...
//  bool wTrig = fb.hlt1 && fb.tos && fb.l1t;
//  bool woTrig = !fb.hlt1 || !fb.tos || !fb.l1t;

  bool wTrig = fGoodHLT;
  bool woTrig = !fGoodHLT;

  bool loPU = (fb.pvn <  9);
  bool hiPU = (fb.pvn > 24);

  bool sfl = (fb.fls3d < 12.); //flight length significance
  bool bfl = (fb.fls3d > 50.);

  bool ldz = (TMath::Abs(fb.dzmin) > 1.2); //1.2 min delta(z) of the next-closest PV
  bool sdz = (TMath::Abs(fb.dzmin) < 0.08); //0.08
...
void plotReducedOverlays::sbsDistributions(string sample, string selection, string what) {
  ...
  for (unsigned int i = 0; i < fDoList.size(); ++i) {
    ...
    if (h) cout << "  Title: " << h->GetTitle() << " with integral: " << h->GetSumOfWeights() << endl;
  }
}
...
void plotReducedOverlays::overlay(string sample1, string sample2, string selection, string what) {
  ...
  for (unsigned int i = 0; i < fDoList.size(); ++i) {
    ...
    if (string::npos != fDoList[i].find("bdt")) doLegend = true;
  ...
  for (int id = 0; id < fDoList.size(); ++id) {
    ...
    if (string::npos != sample1.find("Mc")) {
      ...
      if (string::npos != sample1.find("Sg")) {
        ...
        h1string = "MC simulation";
        // -- change things for the private/official validation
         if (string::npos != sample1.find("Comb") && string::npos != sample2.find("Off")) {
          h1string += " with trig ";
          sprintf(loption1, "p");
          h1->SetMarkerStyle(24);
          h1->SetMarkerSize(1.5);
    ...
    if (string::npos != sample1.find("bupsikData") && string::npos != sample2.find("bupsikData")) {
      h1string += " Low PU";
      //sprintf(loption1, "f");
      h1->SetMarkerColor(kRed);
      h1->SetLineColor(kRed);
