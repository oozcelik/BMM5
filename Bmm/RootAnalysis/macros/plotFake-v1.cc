...
//// At most one of the two tracks must be (mis)identified as global muon. ////
  bool singleFake = (nfakes == 1);
  if (fMode == FAKEPSI) singleFake = true;

//// We compare the distributions in three regions //// 
  for (int i = 0; i < fFakeNtrk; ++i) {
...
//// MC samples is selected with MC truth from (two-body) b-hadron decays. /////
    if (fIsMC) {
      if (fMode == FAKEKS) {
