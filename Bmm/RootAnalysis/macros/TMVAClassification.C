...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   bckinput = TFile::Open( fname );

   TFile *siginput(0);
   TString fname2 = "output.bmmReader.mix-Bs2MuMu.root"; 
   siginput = TFile::Open( fname ); // check if file in local directory exists

   if (!siginput || !bckinput) {
      std::cout << "ERROR: could not open data file" << std::endl;
      exit(1);
   }
   std::cout << "--- TMVAClassification       : Using signal input file: " << siginput->GetName() << std::endl;
   std::cout << "--- TMVAClassification       : Using background input file: " << bckinput->GetName() << std::endl;

   // Register the training and test trees

   TTree *signalTree     = (TTree*)siginput->Get("candAnaMuMu/muonidtree");
   TTree *background     = (TTree*)bckinput->Get("candAnaMuMu/muonidtree");

   TTree *signalTree2     = (TTree*)siginput->Get("candAnaMuMu/events");
   TTree *background2     = (TTree*)bckinput->Get("candAnaMuMu/events");
...
//"gmu1gmid<1 && gmu2gmid<1";
//"chi2LocMom>0 && chi2LocMom<30 && chi2LocPos>0 && chi2LocPos<15 && segComp>0 ";
//&& 0<trkRelChi2<1.2 && 0<kinkFinder<30 && 0<glbKinkFinder<50 && 0<NTrkVHits<35 && 0<valPixHits<8 && 0<LWH<20 && 0<vMuonHitComb<35 && 0<segComp<1.2 && 0<timeAtIpInOutErr<3.5 && 0<innerChi2<3 && 0<outerChi2<4 && 0.7<trkValidFract<1"; // for example: TCut mycutb = "abs(var1)<0.5";
