...
void cleanupTruthMatching(Handle<View<Track> > &hTracks, const MagneticField *magfield) {
  ...
  for (ii = genIdxTracks.begin(); ii != genIdxTracks.end(); ++ii) {
    if (ii->second.size() > 1) {
      if (verbose) {
	gHFEvent->getGenCand(ii->first)->dump(1);
	cout << "--> with " << ii->second.size() << " tracks associated to it, n(simpleTracks) = " << gHFEvent->nSimpleTracks()
	     << ": ";
	...
	ipGen.SetXYZ(tsof.globalPosition().x(), tsof.globalPosition().y(), tsof.globalPosition().z());
      ...
      int pmin(-1);
      for (unsigned int iv = 0; iv < ii->second.size(); ++iv) {
	// cout << "look at simple track " << ii->second[iv] << endl;
	// -- track delta R
	...
	tsof = ipExt.extrapolate(fts, vtx);
	if (!tsof.isValid()) {
	  cout << "problem with extrapolation for simple track " << ii->second[iv] << endl;
	  ipGen.SetXYZ(999., 999., 999.);
	} else {
	...
	if (ii->second[iv] == bestChoice) {
	  if (verbose) cout << " keeping gen index for track " << ii->second[iv] << endl;
	} else {
